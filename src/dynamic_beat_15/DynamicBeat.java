...
public class DynamicBeat extends JFrame {
	...
	private boolean isGameScreen = false; // 게임화면에 넘어왔는지에 대한 함수
	...
	/* 이 게임프로그램이 실행됬을 때 단 하나의 게임만 실행된다.(동시에 여러곡 플레이 X)
	 * 그래서 게임이라는 변수는 이 프로젝트 전체에서 통용이되는 하나의 변수이기에 public static으로 지정해준다.
	 * 이로써 이 game이라는 변수는 프로젝트 전체에서 사용되는 하나의 변수가 되는것이다. 
	 */

	public DynamicBeat() {
		// 생성자는 자신의 클래스와 같은 이름을 가지고 있음
		// trackList를 불러오지 않은 상태에서 버튼들을 불러오게되면 오류가 발생할 수 있기에 맨 위에 올려준다.
		trackList.add(new Track("Aimer - Stardust Title Image.png","Aimer - Stardust Start Image.png",
				"Aimer - Stardust Game Image.png", "Aimer - Stardust Selected.mp3", "Aimer - Stardust.mp3", "Aimer - Stardust"));
		trackList.add(new Track("Alan Walker - Faded Title Image.png","Alan Walker - Faded Start Image.png",
				"Alan Walker - Faded Game Image.png", "Alan Walker - Faded Selected.mp3", "Alan Walker - Faded.mp3","Alan Walker - Faded"));
		trackList.add(new Track("월피스카터-시간의 비, 최종전쟁 Title Image.png","월피스카터-시간의 비, 최종전쟁 Start Image.png",
				"월피스카터-시간의 비, 최종전쟁 Game Image.png", "월피스카터-시간의 비, 최종전쟁 Selected.mp3", "월피스카터-시간의 비, 최종전쟁.mp3", "월피스카터-시간의 비, 최종전쟁"));
		...
		addKeyListener(new KeyListener()); // KeyListener 클래스를 적용
	...
	public void paint(Graphics g) {
		...
		screenDraw((Graphics2D) screenGraphic); // screenGraphic에 어떠한 그림을 그려주나 만드는건 아래에 있음
		// Graphics2D 형변환은 글자가 깨지는걸 막기 위한 변환
		g.drawImage(screenImage, 0, 0, null); // 게임창에 screenImage가 그려짐
	}

	public void screenDraw(Graphics2D g) { // 원래는 Graphics g 였으나 2D로 변형
		g.drawImage(background, 0, 0, null); // introbackground를 0,0의 위치에 그려준다.
		if (isMainScreen) { // 화면이 메인화면일 때 보여주는 그림
			g.drawImage(selectedImage, 340, 100, null);
			g.drawImage(titleImage, 340, 70, null);
		}
		if(isGameScreen) { // 게임화면일 때
			game.screenDraw(g);
		}
		paintComponents(g);
	...
	public void gameStart(int nowSelected, String difficulty) { // 현재선택된곡 난이도에 대한 정보
		...
		game.start();
		setFocusable(true); // 메인화면에 키보드 포커스가 맞춰진다.
		/* 혹시나 프레임에 버튼이나 텍스트필드등 다른 객체들이 배치되어 있다면 프레임에 키입력이 캐치되지 않을 수 있습니다.
		 * 이런 경우 프레임의 setFocusable 를 true 로 해주세요. 
		 * 우리가 어떤 프로그램을 사용할때 거기에 키를 입력하려면 일단 거기로 이동해서 들어가야 하잖아요?
		 * 이걸 '포커스'라고 합니다. 프레임에 포커스를 넣어주면 키처리가 잘 될것입니다. 		 */
